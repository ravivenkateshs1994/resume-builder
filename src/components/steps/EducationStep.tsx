...
function MonthYearSelect({
  ...
  const selectClass =
    "flex-1 border border-slate-200 rounded-lg px-2 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500 bg-white disabled:bg-gray-50 disabled:text-gray-400";
...
export default function EducationStep() {
  ...
  return (
    <div>
              ...
              <div className="p-4 space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input value={e.institution} onChange={(v) => updateEducation(e.id, { institution: v.target.value })} placeholder="e.g. New York University" className="w-full border border-slate-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500" />
                  </div>
                  <div>
                    <label className="block text-xs font-semibold text-slate-600 uppercase tracking-wide mb-1.5">Degree</label>
                    <input value={e.degree} onChange={(v) => updateEducation(e.id, { degree: v.target.value })} placeholder="e.g. Bachelor of Science" className="w-full border border-slate-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500" />
                  </div>
                  <div>
                    <label className="block text-xs font-semibold text-slate-600 uppercase tracking-wide mb-1.5">Field of Study</label>
                    <input value={e.field} onChange={(v) => updateEducation(e.id, { field: v.target.value })} placeholder="e.g. Computer Science" className="w-full border border-slate-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500" />
                  </div>
                  <div>
                    ...
                    <input value={e.gpa || ""} onChange={(v) => updateEducation(e.id, { gpa: v.target.value })} placeholder="e.g. 3.8" className="w-full border border-slate-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500" />
                  </div>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                  ...
                  <div>
                    ...
                    <label className="inline-flex items-center gap-2 mt-2 cursor-pointer select-none">
                      <input type="checkbox" checked={isCurrent} onChange={(ev) => updateEducation(e.id, { endDate: ev.target.checked ? "Present" : "" })} className="rounded border-slate-200 text-violet-600 focus:ring-violet-500" />
                ...
                <div>
                  ...
                  <input value={e.honors || ""} onChange={(v) => updateEducation(e.id, { honors: v.target.value })} placeholder="e.g. Magna Cum Laude, Dean's List" className="w-full border border-slate-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500" />
                </div>
              </div>
            </div>
          );
      ...
      <button onClick={addEducation} className="mt-3 w-full border-2 border-dashed border-slate-200 hover:border-violet-400 text-gray-500 hover:text-violet-600 rounded-xl py-3 text-sm font-medium transition-colors">
        + Add Education
      </button>
            ...
            <div className="p-4 space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input value={c.name} onChange={(v) => updateCertification(c.id, { name: v.target.value })} placeholder="e.g. AWS Solutions Architect" className="w-full border border-slate-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-slate-600 uppercase tracking-wide mb-1.5">Issuing Organization</label>
                  <input value={c.issuer} onChange={(v) => updateCertification(c.id, { issuer: v.target.value })} placeholder="e.g. Amazon" className="w-full border border-slate-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500" />
                </div>
              </div>
              {/* Row 2 */}
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input value={c.credentialId ?? ""} onChange={(v) => updateCertification(c.id, { credentialId: v.target.value })} placeholder="e.g. ABC-123456" className="w-full border border-slate-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500" />
                </div>
                <div>
      ...
      <button onClick={addCertification} className="mt-3 w-full border-2 border-dashed border-slate-200 hover:border-violet-400 text-gray-500 hover:text-violet-600 rounded-xl py-3 text-sm font-medium transition-colors">
        + Add Certification
      </button>
